...
class IsDivisibleBy(Constraint):
    ...
    def is_satisfied(self, bindings: Dict[str, int]) -> bool:
        return (self.value.evaluate(bindings) % self.divisible_by.evaluate(bindings)) == 0
...
class NOf(Constraint):
    ...
    def is_satisfied(self, bindings: Dict[str, int]) -> bool:
        return sum(1 for c in self.sub_constraints if c.is_satisfied(bindings)) == self.n
...
def backtrack(variables: List[str], domains: Dict[str, List[int]], constraints: List[Constraint]) -> Generator[Dict[str, int], None, None]:
    def is_consistent(assignment: Dict[str, int]) -> bool:
        return all(constraint.is_satisfied(assignment)
                    for constraint in constraints
                    if set([v.name for v in constraint.variables()]).issubset(set(assignment.keys())))
    
    def backtrack_helper(assignment: Dict[str, int]) -> Generator[Dict[str, int], None, None]:
        if len(assignment) == len(variables):
            if is_consistent(assignment) and not all(v == 0 for v in assignment.values()):
                print(f"{assignment=}")
...
def find_bindings(variables: List[str], domains: Dict[str, List[int]], constraints: List[Constraint]) -> Generator[Dict[str, int], None, None]:
...
def expression_string(expression: Value, values: dict[str, int], hold_out: Variable | None = None, underline: Variable | None = None) -> str:
